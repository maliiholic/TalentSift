  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-extrabold text-[#0073b1] tracking-tight">Manage Subscriptions</h1>
        <p className="text-sm text-gray-500">View active member tier statuses and cancel premium recruiter subscriptions.</p>
      </div>
        ...
        <table className="w-full table-auto border-collapse">
          <thead className="bg-slate-50 border-b border-gray-150 text-gray-500 text-xs font-bold uppercase tracking-wider">
            <tr>
              ...
              <th className="px-6 py-4 text-left font-semibold">Subscription Type</th>
          ...
          <tbody className="text-sm">
            {loading ? (
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-gray-500 font-medium">Loading subscriptions...</td>
              </tr>
            ) : error ? (
              <tr>
              ...
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-gray-500 font-medium">No active subscriptions found</td>
              </tr>
            ) : (
              subscriptions.map((sub) => (
                <tr key={sub.id} className="border-b border-gray-100 hover:bg-slate-50/50 transition duration-150">
                  <td className="px-6 py-4 font-semibold text-gray-800">{sub.id}</td>
                  <td className="px-6 py-4 text-gray-750 font-medium">{sub.email}</td>
                  <td className="px-6 py-4 text-gray-650 font-medium capitalize">{sub.subscription}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-40 z-50 transition-opacity duration-200">
          <div className="bg-white p-8 rounded-2xl border border-gray-150 shadow-2xl w-96 max-w-sm text-center space-y-4">
            ...
            <p className="text-sm text-gray-500 leading-relaxed">Are you sure you want to cancel this user subscription? The user will immediately revert to the free plan.</p>
