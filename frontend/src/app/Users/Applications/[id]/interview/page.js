...
export default function InterviewPage() {
    ...
    const submitAnswer = async () => {
        ...
        try {
            ...
            // If backend auto-finalized the session on last question, show result immediately
            if (data.finished) {
                setCompleted(true);
            ...
            // If backend returned next_order, advance UI to that question index (server canonical)
            if (data.next_order) {
    ...
    const handleCompleteInterview = async () => {
        ...
        // Stop the timer immediately so the screen freezes at completion time.
        setFinishing(true);
        setTimeLeft(null);
        try {
            ...
            // If completion fails, allow the user to retry and re-enable timer state.
            setFinishing(false);
    ...
    return (
        <div className="min-h-screen bg-[#F4F2EE] py-8 px-4">
            <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-lg p-8">
                ...
                <h2 className="text-xl font-semibold text-gray-800 mb-4">{currentQuestion.text || currentQuestion.question}</h2>
