...
def verify_payment_prac(request):
    ...
    try:
        ...
        if session.payment_status == 'paid':
            ...
            elif hasattr(raw_metadata, 'keys'):
                metadata = {key: raw_metadata[key] for key in raw_metadata.keys()}
            else:
                metadata = {}

            user_id = metadata.get('user_id')
            practice_type = metadata.get('practice_type', 'practice')
            ...
            return Response({
                'status': 'Payment successful, practice subscription updated' if not created else 'Payment successful, new practice subscription created'
            })
        ...
        logger.exception("Unexpected error in verify_payment_prac")
        return Response({'error': 'An unexpected error occurred', 'details': str(e)}, status=500)
